...
def draw_YLPs(x, YLPs, ymax=None, draw_center=False, cluster_center=None,
    ...
    YLPs = np.array(YLPs)
    x = np.array(x)
    assert len(x) == YLPs.shape[1]

    for YLP in YLPs:
        if draw_center:
            plt.plot(x, YLP, color='#C8C8FF', linewidth=0.3)
        else:
            plt.plot(x, YLP, linewidth=0.3)

    ax = plt.gca()
    if draw_center:
        if np.any(cluster_center) != None:
            plt.plot(x, cluster_center, color='#4B0082',
                    linewidth=0.4, label='Cluster Center')
            plt.plot(x, YLPs.mean(axis=0), color='#8B0000',
                    linewidth=0.4, label='Mean YLP')
        else:
            plt.plot(x, YLPs.mean(axis=0), color='#8B0000',
                    linewidth=0.4, label='Cluster Center')
...
if __name__ == '__main__':
    ...
    # draw_YLPs(x, y)
    draw_YLPs(x, y, cluster_center=np.random.rand(LEN)+1)
    # draw_YLPs(x, y, cluster_center=np.random.rand(LEN), filepath='./负荷曲线图YLPs示例.svg')
